...
### 文档协同（执行强制）
- `DevPlan.md`：定义“做什么、分阶段怎么做”（计划层）。
- `DevRule.md`：定义“具体如何做、哪些不能做”（规则层）。
- `RefactorRecord.md`：记录“这次具体改了什么、完成了哪些阶段目标、风险与验证”（执行记录层）。
- 后续任何“按本计划推进的重构任务”，都必须同步更新以上三份文档。
...
### 6.8 列表渐入层次（新增）
- `index.php` / `archive.php` 列表卡片在大部分允许场景都应有可感知的渐入，动效语言为 `fadeInUp`（淡入 + 上移）。
- 渐入强度可以略明显，不要求过度克制，但必须保持文字可读性与滚动稳定性。
- 首屏渐入不依赖 Swup。即使 Swup 关闭或初始化失败，也应保持一致的列表进入观感。
- 命中形态 A（VT 共享元素）返回列表时：VT 进行阶段只展示共享主体 morph；其余卡片与列表辅助区块需先隐藏。
- VT 完整结束后，其余卡片与辅助区块再开始 `fadeInUp` 介入，形成“先主体、后列表”的层次。
- `prefers-reduced-motion: reduce` 时关闭位移/缩放，仅保留极短淡入或直接无动画。

### 6.9 页面模板动画一致性（新增）
- `page.php` 与任意自定义页面模板（例如 `archives.php`）必须被视为同一种 `page` 动画语义，不允许分叉策略。
- 页面进出动画应绑定统一结构标记（例如 `data-ps-page-shell`），而不是依赖模板文件名或偶发 class 组合。
- 动画校验标准：同入口、同方向、同缩放曲线、同阶段时序（进入/退出/预载）在 `page.php` 与自定义页面模板上保持一致。
- 视觉一致性补充：当页面内容高度差异较大（如时间线归档页）时，仍需保持“可感知缩放”；应通过统一 `transform-origin` 与 page 专属缩放参数消除“只剩平移感”的偏差。

### 6.10 右栏结构规范化（新增）
- 右侧边栏必须是标准布局列（语义容器中的独立列），不得通过 `display: inline` 或 body 偏移量“伪造三栏”。
- `#swup` 只允许包裹正文主内容列，右侧边栏必须在 `#swup` 外，避免页面切换时被替换/丢失。
- 桌面端采用稳定三栏语义（左导航/中正文/右边栏），移动端可降级为隐藏或折叠右栏，但结构语义不变。
- 现有 TOC 粘性行为需适配栏内布局（栏内 sticky），避免依赖全局 fixed 浮动定位。
