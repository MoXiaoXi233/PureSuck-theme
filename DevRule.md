...
### 16.1 动画状态机约束
- CSS 状态类总数不得超过 10 个（核心 6 个 + 页面类型 3 个 + 预留 1 个）。
- 时间常量统一定义在 `:root`，JS 中不得硬编码时长。
- 计时器必须统一管理，不得分散在多个对象中。

### 16.2 缓存结构约束
- JS 端：单个模块的状态变量不得超过 5 个；Map/Set 总数不得超过 2 个。
- PHP 端：缓存层数不得超过 2 层（内存 + 可选文件）；cache key 组成部分不得超过 3 个。

### 16.3 全局命名空间约束
- 禁止在 `window` 上直接挂载业务对象（如 `window.mediumZoomInstance`）。
- 所有对外 API 必须收敛到 `window.PS` 下。
- `window.PS` 的一级属性不得超过 10 个。

### 16.4 分段动画约束
- 禁止将完整卡片内部内容拆分为多个区块做分段渐入/退出。
- 列表卡片禁止使用 stagger 延迟（nth-of-type 递增 delay）。
- 动画延迟仅允许用于：区分进入/退出阶段、区分共享/非共享元素。
