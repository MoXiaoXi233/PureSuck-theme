(() => {
    class OwO {
        ...
        init(option) {
            ...
            // ★ 性能优化：首次只渲染骨架，表情内容延迟到打开时渲染
            this._isContentRendered = false;

            // 创建骨架：只有激活按钮和空的面板结构
            let html = `<div class="OwO-logo"><span>${option.logo}</span></div><div class="OwO-body"><div class="OwO-panel"><div class="OwO-viewport"></div><div class="OwO-bar"><ul class="OwO-packages">`;
        ...
        // ★ 新增：首次打开时渲染所有表情内容
        _renderContent() {
            if (this._isContentRendered) return;
            ...
            this.packages.forEach((pkg, idx) => {
                ...
                ul.className = `OwO-items OwO-items-${type}`;
                ul.setAttribute('data-pkg-idx', idx);
                if (idx !== 0) ul.setAttribute('hidden', '');
                ...
                items.forEach(item => {
                    ...
                    if (type === 'image') {
                        ...
                        img.alt = text;
                        img.loading = 'lazy';
                        if (pd.width) img.style.width = pd.width;
                        li.appendChild(img);
                    } else {
                        li.textContent = icon;
                    }
